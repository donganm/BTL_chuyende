...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Lấy giá trị article_id và content từ POST
    $article_id = isset($_POST['article_id']) ? (int)$_POST['article_id'] : 0;
    $content = isset($_POST['comment']) ? trim($_POST['comment']) : '';
    $username = isset($_SESSION['username']) ? $_SESSION['username'] : 'Ẩn danh';

    // Kiểm tra article_id hợp lệ (không phải 0 và không NULL)
    if ($article_id <= 0 || empty($content)) {
        echo "Dữ liệu không hợp lệ!";
        exit();
    }

    // Thực hiện chèn bình luận vào database
    $sql = "INSERT INTO comments (article_id, username, content, created_at) VALUES (?, ?, ?, NOW())";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("iss", $article_id, $username, $content);

    if ($stmt->execute()) {
        // Sau khi chèn thành công, chuyển hướng về trang xem bài viết
...
<body>
    <div class="container">
        ...
        <div class="comment-section">
            ...
            $stmt->bind_param("i", $article_id); // Sử dụng $article_id thay vì $id
            $stmt->execute();
